...
select year(start_timestamp) as years, month(start_timestamp) as months, sum(total_paid) as revenue from rental
group by year(start_timestamp) ,month(start_timestamp)
union all
select year(start_timestamp) as years, null as months, sum(total_paid) as revenue from rental
group by year(start_timestamp)
union all
select null as years, null as months, sum(total_paid) as revenue from rental
order by years,months;

/* Emily would like data about memberships purchased in 2023, with
...
-- total membership revenue specific for each revenue type
-- total membership reveneue
select name, month(start_date) as month, sum(total_paid) as total_revenue
 from membership m join membership_type mt on mt.id=m.membership_type_id
 group by name,month
 union all
 select name, null as month, sum(total_paid) as total_revenue
 from membership m join membership_type mt on mt.id=m.membership_type_id
 group by name,month
 union all
  select null as name, null as month, sum(total_paid) as total_revenue
 from membership m join membership_type mt on mt.id=m.membership_type_id
 order by name,month;
...
select count(*) as number_of_bikes,category from bike -- Use COUNT(*) or COUNT(1) for general row counting. Use COUNT(column_name) only if you specifically want to ignore rows with NULL values in that column.
...
select c.name, count(m.membership_type_id) as membership_count from customer c left join membership m on c.id=m.customer_id
group by name
...
-- OR
SELECT category,
    SUM(status = 'available') AS available_bikes_count,
    SUM(status = 'rented') AS rented_bikes_count
FROM bike
GROUP BY category;
...
*/
select * from membership;
select year(start_date) as year, month(start_date) as month,mt.name, sum(total_paid) as total_revenue
...
order by year(start_date), month(start_date),mt.name;
